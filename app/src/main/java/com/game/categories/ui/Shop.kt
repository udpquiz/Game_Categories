    ...
    private fun getFetchAllCategory(){
        ...
        apiInterface.getFetchAllCategory().enqueue(object : Callback<MainData> {
            override fun onResponse(call: Call<MainData>, response: Response<MainData>) {
                if (response.isSuccessful){if(HomeActivity.checkBox.isChecked){
                    adapter = VerticalAdapter(this@Shop,response.body()?.data?.tags!!,response.body()?.data?.data!! , true)
        ...
        apiInterface.getFetchAllCategory().enqueue(object : Callback<MainData> {
            override fun onResponse(call: Call<MainData>, response: Response<MainData>) {
                if (response.isSuccessful){
                    val tslist = loadTimeStampList(this@Shop)
                    if (tslist != null) {
                        for (ts in tslist) {
